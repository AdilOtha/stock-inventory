...
?>
<?php 
      if(isset($_POST['addOrdersElecbtn'])){
        $a=$_POST['oeid'];
        $b=$_POST['sid'];
        $c=$_POST['pid'];
        $d=$_POST['order_date'];
        ...
        $insert="insert into orders_elec(oeid,sid,pid,order_date,ordered_quantity,paid_amount,order_status) values('$a','$b','$c','$d','$e','$f','$g');";
      ...
      if(isset($_REQUEST['id'])){
        $delid=$_REQUEST['id'];
        //echo $delid;
        $delete="DELETE FROM orders_elec WHERE oeid='$delid'";
...
<div class="card">
    <div class="card-header"><i class="fa fa-edit"></i>Manage Electronics Orders<br>  
    </div>
    <div class="card-body">      
      <div class="div-action" style="padding-bottom: 20px">
        <button class="btn btn-primary" data-toggle="modal" data-target="#addOrdersElec"><i class="fa fa-plus"></i>Place Order</button>                
      </div>
<?php 
      $getOrdersElec="SELECT * FROM orders_elec";      
      $result=$connect->query($getOrdersElec);      
      print("<div class='row'>
        <div class='col-sm-12'>
      ...
      function setGtDa(string $oeid) {
        try {
            ...
            $sql = ' CALL `setGrandTotal_DueAmount_Electronics`(:id, @p1, @p2, @p3);';
     
            // prepare for execution of the stored procedure
            $stmt = $pdo->prepare($sql);
     
            // pass value to the command
            $stmt->bindParam(':id', $oeid, PDO::PARAM_STR);
      ...
      while ($row=$result->fetch_assoc()) {    
        ...
        print("<tr>
              ...
              <td>".$h."</td>
              <td>".$i."</td>");
              print("<td><button class='btn btn-danger' data-toggle='modal' id=".$oeid." data-target='#delOrdersElec' onClick='return_del_id(this.id)'><i class='fa fa-trash'></i>Delete</button></td></tr>");              
      }
      print("</tbody>
        </table>
        </div>
...
<div class="modal fade" tabindex="-1" role="dialog" id="addOrdersElec">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="container">
          <form action="orders_elec.php" class="needs-validated" id="modal-form" method="POST">
              <div class="form-group">
                <label for="oeid">OrderID:</label>
                <input type="text" class="form-control" id="oeid" placeholder="Enter OrderID" name="oeid" required>
              </div>
              <div class="form-group">
      ...
      <div class="modal-footer">
        <button type="submit" class="btn btn-primary" id="addOrdersElecbtn" name="addOrdersElecbtn" form="modal-form">Save Changes</button>
...
<div class="modal fade" tabindex="-1" role="dialog" id="delOrdersElec">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
      ...
      <div class="modal-footer">
        <a class="btn btn-danger" id="delOrdersElecLink">Confirm</a>
...
<script>
  ...
  {
      var x="orders_elec.php?id=";
      var y=x.concat(clicked_id);
      document.getElementById('delOrdersElecLink').setAttribute('href',y);
  }
